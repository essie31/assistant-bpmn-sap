...
if check_password():
    ...
    def generate_full_analysis(tasks_text, flows_text):
        ...
        * **Tâche BPMN :** [Nom]
        ...
        ATTENTION EXTRÊME : Tu DOIS évaluer TOUTES les tâches listées au début du prompt, sans en oublier une seule.
        Colonnes du tableau : 
        `Tâche BPMN` | `Big Data` | `Robots` | `Simulation` | `Intégration` | `IIoT` | `Cyber.` | `Cloud` | `Additif` | `RA` | `Justification`.
        CONTRAINTE DE MÉMOIRE STRICTE : Pour que la génération ne soit pas coupée, la colonne `Justification` doit contenir UNIQUEMENT 3 ou 4 mots maximum en style télégraphique (ex: "Fort potentiel analytique", "Automatisation SAP vitale", "Non pertinent ici").
        ...
        ```
        """
        # Appel avec les 8192 tokens pour avoir assez d'espace pour le grand tableau
            ...
            if st.button("Lancer l'évaluation complète", type="primary"):
                with st.spinner("Analyse et génération de la Matrice I4.0 en cours avec Gemini 2.5 Flash..."):
                    ...
                    if tasks_text is None:
                        ...
                        # Affichage du rapport complet en PLEINE LARGEUR (pour que la Matrice ait de la place)
