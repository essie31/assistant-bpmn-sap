...
from google import genai # NOUVELLE BIBLIOTHÈQUE GOOGLE
...
# ==========================================
# 🔑 API GEMINI (NOUVEAU CLIENT)
# ==========================================
API_KEY = st.secrets["API_KEY"]
client = genai.Client(api_key=API_KEY) # NOUVELLE FAÇON DE SE CONNECTER
...
"""
    try:
        # NOUVEL APPEL AVEC LE NOUVEAU CLIENT OFFICIEL
        ...
        return f"Erreur avec la nouvelle API : {str(e)}"
...
if check_password():
    ...
    if uploaded and st.button("Analyser"):
        with st.spinner("Analyse en cours via la nouvelle API Google..."):
            tasks, flows = parse_bpmn(uploaded)
            report = generate_analysis(tasks, flows)

            if "Erreur avec la nouvelle API" in report:
                st.error(report)
            else:
