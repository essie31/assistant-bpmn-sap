...
@media print {
    ...
    header, footer, .stButton, .stFileUploader, .stTextInput {
        display: none !important;
    }
    /* Forcer l'impression des couleurs d'arrière-plan (pour le radar) */
    * {
        -webkit-print-color-adjust: exact !important;
        print-color-adjust: exact !important;
    }
    /* Éviter que les tableaux soient coupés au milieu d'une page */
...
if check_password():
    ...
    def generate_full_analysis(tasks_text, flows_text):
        ...
        * **Tâche BPMN :** [Nom]
        ...
        Pour chaque tableau, évalue les TÂCHES PRINCIPALES avec les colonnes : `Tâche BPMN` | `Score (1-5)` | `Justification`.

        ### 5. SCORES_JSON
        À la toute fin, inclut un bloc JSON valide avec la note globale de 1 à 5 du processus entier pour les 9 piliers.
        ```json
        {{
          "Big Data": 2,
          "Robots Autonomes": 1,
    ...
    def draw_radar_chart(json_str):
        ...
        try:
            ...
            fig = px.line_polar(df, r='r', theta='theta', line_close=True, range_r=[0,5], 
                                ...
                                markers=True)
            fig.update_traces(fill='toself', line_color='#ff7f0e') # Couleur orange pour différencier
            return fig
        except Exception as e:
            return None
            ...
            if st.button("Lancer l'évaluation complète", type="primary"):
                with st.spinner("Analyse et génération du radar en cours..."):
                    ...
                    if tasks_text is None:
                        ...
                        # Affichage du rapport en pleine largeur pour faciliter l'impression
